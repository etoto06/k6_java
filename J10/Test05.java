...
public class Test05 {
	...
	public static void main(String[] args) {
		...
		for (String element : list) { //확장 for문이 중요한이유 커서가 있다 
		...
		list.add(1, "LA");
		print(1, list); 
		list.addFirst("런던"); // 링크드리스트 처음부분에 런던 넣어라는 뜻 
		print(2, list);
		list.addLast("서울");
		print(3, list);

		list.offer("파리"); //offer 하면 add 하고 거의 같은것 
		print(4, list);
		list.offerFirst("로마");
		print(5, list);
		list.offerLast("베른");
		...
		System.out.println("7 : " + list.peek());      //피크는 ㅇ
		...
		List<String> list2 = Arrays.asList("북경", "상해"); // 배열을 리스트로 만드는것 
		list.addAll(list2);
		//list.sort();/Linked List의 sort는 지원되지 않음
		Object obj[] = list.toArray();
		System.out.println("24 : " + Arrays.toString(obj)); //리스트를 배열로 만드는것 
